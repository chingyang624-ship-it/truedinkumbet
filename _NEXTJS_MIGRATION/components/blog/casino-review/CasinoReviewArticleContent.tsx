...
const nonIndexableArticlePatterns = [
  /pagcor/i,
  /license-gamingcuracao/i,
  /gaming cura[cç]ao/i,
  /superannuation/i,
  /tax refund/i,
  /use vpn/i,
...
export function isArticleIndexable(article: Article) {
  const nonIndexableTitlePatterns = [
    /\b2021\b/i,
    /\b2025\b/i,
    /affiliate/i,
    /brand ambassador/i,
  ...
  return !nonIndexableTitlePatterns.some((pattern) => pattern.test(article.title))
    && !nonIndexableArticlePatterns.some((pattern) =>
...
export default function CasinoReviewArticleContent({ slug }: { slug: string }) {
  ...
  // Migrated WordPress bodies often contain another H1. The title below is the
  // single page H1, so demote embedded H1 tags to H2 before rendering.
  const articleContent = article.content
    ?.replace(/<h1(\s[^>]*)?>/gi, '<h2$1>')
    .replace(/<\/h1>/gi, '</h2>')
    .replace(
      /href="\/blog\/category\/(?!casino-review\/)[^"]+"/gi,
      'href="/blog/category/casino-review/"'
    )
    .replace(
      /href="\/blog\/(?:live-casino-review|poker-review|lottery-review)\/[^"]+"/gi,
      'href="/blog/category/casino-review/"'
    );
